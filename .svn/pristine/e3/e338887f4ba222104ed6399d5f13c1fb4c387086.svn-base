<template>
    <view class="content">
        ...
        <view class="main">
            <x-input type="tel" v-model="sendData.phone" @input="input" :disabled="type == 1 ? false : true" showNationCode='true' placeholder="请输入手机号"></x-input>
            <x-input type="captcha" maxlength="6" v-model="sendData.verifyCode" placeholder="手机验证码" captcha-type="2" :captchaMobile="sendData.phone"></x-input>
            <x-input class="item" type="password" placeholder="密码长度8-16位，须数字，字母组合" v-model="sendData.password"></x-input>
            <x-input class="item" type="password" placeholder="确认密码" v-model="sendData.passwordConfirm"></x-input>

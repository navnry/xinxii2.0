<template>
    <view class="lazy-loadimage" :id="uid">
        <image lazy-load class="origin-img" :src="imageSrc" :mode="mode"
...
export default{
    props:{
        ...
        scrollTop:{ // 传入滚动值监听并触发组件
            type: Number,
        },
        loadingMode:{ // loading加载中的占位图效果
            type: String,
            default:'looming-gray'
        },
        openTransition:{ //是否开启加载成功后的渐现过渡效果
            type: Boolean,
            default:true,
        },
        viewHeight:{ // 可视区域高度
...
<style scoped>
    ...
    .loadfail-img{
      width: 100%;
      height: 100%;
      background: url('~@/static/lazy-loadimage/loadfail.png') no-repeat center;
      background-size: 50%;
    }
    .loading-img{
      width: 100%;
    ...
    /* 转圈 */
    .spin-circle{
      background: url('~@/static/lazy-loadimage/loading.gif') no-repeat center;
